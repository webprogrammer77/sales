...
$query_sales_rules = "SELECT value FROM belotorg_settings WHERE type='Download_over_sport'";
...
{
$query_update_2 = "UPDATE  belotorg_settings SET `value` = '2' WHERE `type` = 'Download_over_sport'";
...
$rules_id   			= $row_sales_rules['id'];
$price_cost_in   		= $row_sales_rules['price_cost_in'];
$product_id_not_in		= $row_sales_rules['product_id_not_in'];
$category_id_not_in		= $row_sales_rules['category_id_not_in'];

 
if  (!empty($sale_a)) 
{
	$select_tov_sales_not_empty = "SELECT ep.tov_id, ep.tov_name, ecp.pic_name, ep.tov_price, ". $sale_a . " , ". $sale_b . ", ". $rules_id ." FROM ep LEFT JOIN e_cc_pics AS ecp ON (ep.tov_id=ecp.tov_id ) WHERE ep.tov_quantity>0 AND ep.tov_id NOT IN (SELECT product_id FROM sales)";
	...
	{
		$where_condition_sales_not_empty .=" AND ep.tov_cat_id NOT IN(". $category_id_not_in . ")";
	}	
	if  (!empty($product_id_not_in)) 
	{
		$where_condition_sales_not_empty .=" AND ep.tov_id NOT IN(".$product_id_not_in. ")";
	}
	...
	if  (!empty($price_from)) 
	{
		if($price_cost_in == 1)
		{
		  $where_condition_sales_not_empty .=" AND ep.price_cost>=".$price_from;
		}
		else
		{	
	...
	if  (!empty($price_to)) 
	{
		if($price_cost_in == 1)
		{
		  $where_condition_sales_not_empty .=" AND ep.price_cost<=".$price_to;
		}
		else
		{	
...
$query_insert_sales_not_empty="INSERT INTO sales (product_id, `name`, `image`, price, sale_a, sale_b, rules_id) ".$select_tov_sales_not_empty;
echo $query_insert_sales_not_empty.";\n";
		
	
mysql_query($query_insert_sales_not_empty) or die(mysql_error());

	$query_update_sales_not_empty = "UPDATE sales, ep SET `sale_a` =".$sale_a.", `sale_b` = ".$sale_b.", `rules_id` = ".$rules_id." WHERE  ep.tov_id=sales.product_id AND ep.tov_quantity>0 AND sales.`sale_b`<".$sale_b;
...
{
	$select_tov = "SELECT ep.tov_id, ep.tov_name, ecp.pic_name,  ep.tov_price, FLOOR((1-((ep.price_cost*(1+(".$markup."/100)))/ep.tov_price))*100) AS sale_a, FLOOR((1-((ep.price_cost*(1+(".$markup."/100)))/ep.tov_price))*100) AS sale_b, ".$rules_id." FROM ep LEFT JOIN e_cc_pics AS ecp ON (ep.tov_id=ecp.tov_id ) WHERE ep.tov_quantity>0 AND ep.tov_id NOT IN (SELECT product_id FROM sales)";
	...
	if  (!empty($category_id_not_in)) 
	{
		$where_condition .=" AND ep.tov_cat_id NOT IN(".$category_id_not_in . ")";
	}
	if  (!empty($product_id_not_in)) 
	{
		$where_condition .=" AND ep.tov_id NOT IN(".$product_id_not_in . ")";
	}
	if  (!empty($manufacturer_id)) 
	{
	...
	if  (!empty($price_from)) 
	{
		if($price_cost_in == 1)
		{
		  $where_condition .=" AND ep.price_cost>=".$price_from;
		}
		else
		{	
		  $where_condition .=" AND ep.tov_price>=".$price_from;
		}
	}
	...
	if  (!empty($price_to)) 
	{
		if($price_cost_in == 1)
		{
		  $where_condition .=" AND ep.price_cost<=".$price_to;
		}
		else
		{	
		  $where_condition .=" AND ep.tov_price<=".$price_to;
		}
	}
...
echo $select_tov;
	$query_insert = "INSERT INTO sales (product_id, `name`, image, price, sale_a, sale_b, rules_id) ".$select_tov;
echo $query_insert.";\n";
	
mysql_query($query_insert)or die(mysql_error());

	$query_update = "UPDATE sales, ep SET `sale_a` =FLOOR((1-((ep.price_cost*(1+(".$markup."/100)))/ep.tov_price))*100) , `sale_b` = FLOOR((1-((ep.price_cost*(1+(".$markup."/100)))/ep.tov_price))*100), rules_id = ".$rules_id." WHERE ep.tov_id=sales.product_id AND ep.tov_quantity>0 AND sales.`sale_b`<  FLOOR((1-((ep.price_cost*(1+(".$markup."/100)))/ep.tov_price))*100)";
...
$query_update_3 = "UPDATE belotorg_settings SET `value` = '3' WHERE `type` = 'Download_over_sport'";
