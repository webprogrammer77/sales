...
/*----roistat------*/
if($_POST['roistat_visit'])
{
	$roistat_visit   = clean($_POST['roistat_visit']);	
}
/*----/roistat------*/
  ...
  $price = (int)$price;  
	...
	if(!preg_match("#^((8|\+7)[\- ]?)?(\(?\d{3}\)?[\- ]?)?[\d\- ]{7,11}$#", $_POST['phone'])){
		...
		if($sale !=0 && $price !=0) {
			...
			$query_alter_roistat = "ALTER TABLE ".$dbName.".`sales_orders` ADD `roistat` int(7) DEFAULT NULL AFTER `url`";
			//mysql_query($query_alter_roistat) or die(mysql_error());
			...
			`roistat` int(7) DEFAULT NULL,
			PRIMARY KEY(`id`)
			)ENGINE=InnoDB DEFAULT CHARSET=utf8";
			mysql_query($query_create) or die(mysql_error());
			$query_add = "INSERT INTO ".$dbName.".sales_orders (`order_date`, `name`,`sku`,`price`,`sale`,`quant`,`phone`,`url`,`roistat`)  VALUES ('".$date_order."','".$name."','".$sku."','".$price."','".$sale."','".$quant."','".$phone."','".$url."','".$roistat_visit."')";
			//echo $query_add;
