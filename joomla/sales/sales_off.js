...
jQuery(document).ready(function (jQuery) {
	...
	if(productID != undefined){
		saleButton = jQuery('#sale-button-product');
	}else{
		saleButton = jQuery('#sale-button');
	}
...
/*--Обновление старых товаров в LocaStorage--*/		
			...
			if (productID != undefined ) {
				...
				setTimeout(function () {					
					if (saleOk == "OK") {				
						addHtml(productID);
						setTimeout(function (){			
							showSales(productID, popupSale, slider, saleButton, buttonOrder);
							}, 300);
						}
					}, 300);
			}else{
	...
	*/
	...
	jQuery(document).mouseleave(function (e) {
		var slMask = 'sal_';
		var alertwin = localStorage.getItem(showMask + productID);
		if (e.clientY < 0 && alertwin != 'yes' && alertStop != 'yes' && productID != undefined) { /*-----можно заменить проверку productID на cartURL-----*/
			// jQuery('#elements').children('.popup-sale__items').remove();
			//	if (lsLen > 0) {
			// for (var i = 0; i < lsLen; i++) {
	...
	if (lsLen > 0) { //проверяем есть ли значения в localstorage		
		...
		for (var i = 0; i < localStorage.length; i++) { // перебираем все данные в хранилище
			...
			if (key.indexOf(slMask) == 0 && localStorage.getItem(key) != 'offer' && localStorage.getItem(key) !== 'undefined') {
				...
				// карточка товара		
				var saleHTML = '<div  class="popup-sale__items" data-price ="' +    newprice + '" data-sale ="' + 
					sale + '" data-view ="' + 
					date + '" data-id="' + 
					id + '" data-sku="' + 
					sku + '"><div class="popup-sale__pic"><img class="popup-sale__img" src="' +
		...
		if(productID !==undefined){
			slider.children('.popup-sale__items:gt(0)').remove();
		}else{
...
function showSales(productID, popupSale, slider, saleButton, buttonOrder) {
	var showMask = 'show_';
