...
SELECT ep.tov_id, ep.tov_name, ec.pic_name, ep.tov_price, 5, 10 
FROM ep LEFT JOIN e_cc_pics AS ec ON (ep.tov_id=ec.tov_id )
...
INSERT INTO sales (product_id, `name`, image, price, sale_a, sale_b) 
SELECT ep.tov_id, ep.tov_name, ec.pic_name, ep.tov_price, 5, 10 
FROM ep CROSS JOIN e_cc_pics AS ec ON (ep.tov_id=ec.tov_id )
WHERE ep.tov_quantity>0
AND ep.manufac_id=1
AND ep.tov_price >= 9000
AND ep.tov_id NOT IN (SELECT product_id FROM sales);;;

INSERT INTO sales (product_id, `name`, image, price, sale_a, sale_b) 
SELECT ep.tov_id, ep.tov_name, ec.pic_name, ep.tov_price, 3, 5 
FROM ep LEFT JOIN e_cc_pics AS ec ON (ep.tov_id=ec.tov_id )
WHERE ep.tov_quantity>0
...
INSERT INTO sales (product_id, `name`, image, price, sale_a, sale_b) 
SELECT ep.tov_id, ep.tov_name, ec.pic_name, ep.tov_price, 15, 16 
FROM ep LEFT JOIN e_cc_pics AS ec ON (ep.tov_id=ec.tov_id )
WHERE ep.tov_quantity>0
AND ep.manufac_id= 237
AND ep.tov_price >= 10000
AND ep.tov_id NOT IN (SELECT product_id FROM sales);;;

INSERT INTO sales (product_id, `name`, image, price, sale_a, sale_b) 
SELECT ep.tov_id, ep.tov_name, ec.pic_name, ep.tov_price, 4, 5 
FROM ep LEFT JOIN e_cc_pics AS ec ON (ep.tov_id=ec.tov_id )
WHERE ep.tov_quantity>0 
AND ep.tov_price >= 10001
AND ep.tov_id NOT IN (SELECT product_id FROM sales);;;

INSERT INTO sales (product_id, `name`, image, price, sale_a, sale_b) 
SELECT ep.tov_id, ep.tov_name, ec.pic_name, ep.tov_price, 2, 3
FROM ep LEFT JOIN e_cc_pics AS ec ON (ep.tov_id=ec.tov_id )
WHERE ep.tov_quantity>0
AND ep.tov_price <= 10000
AND ep.tov_id NOT IN (SELECT product_id FROM sales);;;